...
public class RolServiceImpl implements RolService {
	...
	public List<RoleDTO> findByUser(Long idUser) {
		...
		if(optRol.isPresent()) {
			return optRol.get().getRoles().stream().map(rol->modelMapper.map(rol, RoleDTO.class)).collect(Collectors.toList());
		}
	...
	public RoleDTO findById(Long idRol) {
		...
		if(optPer.isPresent()) {
			return modelMapper.map(optPer.get(), RoleDTO.class);
		}
		return null;
	}
	...
	public List<RoleDTO> findAll() {
		return roleRepository.findAll().stream().map(rol->modelMapper.map(rol, RoleDTO.class)).collect(Collectors.toList());
	}
